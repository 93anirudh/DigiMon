...
declare global {
  interface Window {
    ...
    electronAPI: {
      ...
      onStep: (cb: (step: { type: string; toolName?: string; toolArgs?: any; result?: string; iteration?: number }) => void) => void
      ...
      onQuotaHit: (cb: (data: { from: string; hasGrok: boolean; hasGemini: boolean }) => void) => void
      getMcpConfig: () => Promise<any>
      getMcpStatus: () => Promise<any[]>
      toggleMcpServer: (serverId: string, enabled: boolean) => Promise<boolean>
      enableMcpWithEnv: (serverId: string, envValues: Record<string, string>, serverConfig: any) => Promise<boolean>
